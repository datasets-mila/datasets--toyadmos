...
def extract_7zip(archive, output_dir):
    subprocess.run(["7z", "x", archive, "-y", "-o" + output_dir], check=True)
...
def extract_archive(archive, output_dir=None, delete_archive=False):
    ...
    try:
        ...
        if archive.endswith(".tar") or archive.endswith(".tar.gz"):
            ...
            extract_7zip(archive, output_dir)
...
for directory in directories[args.start:args.end]:
    ...
    archives += glob.glob(os.path.join(directory, "*.7z"))
